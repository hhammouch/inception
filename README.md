...
This project is a System Administration related exercise aimed at broadening knowledge of system administration by using Docker[cite: 1]. It consists of setting up a small infrastructure composed of different services (NGINX, MariaDB, and WordPress) under specific rules within a personal virtual machine[cite: 1]. 
...
*   **Environment Variables:** Used to configure standard settings (like domain names) across containers via a `.env` file[cite: 1]. 
*   **Secrets:** Strongly recommended for storing confidential information (like database passwords or API keys)[cite: 1]. Unlike standard environment variables, properly configured secrets are not easily exposed in process lists or logs.

**Docker Network vs Host Network:**
*   **Host Network:** Binds the container directly to the host machine's network stack (e.g., `network: host`), which provides no isolation and is strictly forbidden in this project[cite: 1].
*   **Docker Network:** Creates an isolated internal network that establishes a secure connection between containers[cite: 1]. It allows WordPress to communicate with MariaDB internally, while only NGINX is exposed to the outside host via port 443[cite: 1].

**Docker Volumes vs Bind Mounts:**
*   **Bind Mounts:** Rely on the host machine's specific directory structure and file paths, which can cause portability issues. They are not allowed for the persistent storage in this project[cite: 1].
*   **Docker Volumes:** Native named volumes managed directly by Docker. They are required for this project to persistently store the WordPress database and website files inside the `/home/<login>/data` directory on the host machine[cite: 1].
...
### Prerequisites
*   A Virtual Machine with Docker and Docker Compose installed[cite: 1].
*   Your local hosts file must be configured so that your domain name (`hhammouc.42.fr`) points to your local IP address[cite: 1].
*   Required data directories must be created at `/home/<hhammouc>/data/wordpress` and `/home/<hhammouc>/data/mariadb`[cite: 1].

### Execution
1.  Clone the repository and ensure your `.env` file is properly configured with your credentials[cite: 1].
2.  Run the following command at the root of the directory to build and start the infrastructure[cite: 1]:
    ```bash
    make all
    ```
3.  The NGINX web server will be accessible solely via HTTPS on port 443[cite: 1].
